...
float math::PointLineDistance(Vector2& p, Vector2& p0, Vector2& p1)
{
	...
	if (l2 == 0.0) return Vector2::Length(p - p0);

	float t = Vector2::Dot(p - p0, p1 - p0) / l2;
	if (t < 0.0) return Vector2::Length(p - p0);
	else if (t > 1.0) return Vector2::Length(p - p1);
...
math::Ray math::CreateRay(Vector2& wndpos, Vector2& window, Matrix& projection, Matrix& view, Matrix& world)
{
	using namespace DirectX;

	XMVECTOR NearScreenSpace = XMVectorSet(wndpos.x, wndpos.y, 0.0f, 1.0f);
	XMVECTOR FarScreenSpace = XMVectorSet(wndpos.x, wndpos.y, 1.0f, 1.0f);

	XMVECTOR NearObjectSpace = XMVector3Unproject(NearScreenSpace, 0.0f, 0.0f, 
		window.x, window.y, 0.0f, 1.0f, projection, view, world);
	XMVECTOR FarObjectSpace = XMVector3Unproject(FarScreenSpace, 0.0f, 0.0f, 
		window.x, window.y, 0.0f, 1.0f, projection, view, world);
...
bool math::RayQuadIntersection(const Ray& ray, const Quadrilateral& quad, float& t, float& u, float& v)
{
	...
	{
		u = alpha;
		if (abs(beta_11 - 1) < cEpsilon) v = beta; // q is a parallelogram
...
bool triangle_intervals(std::array<math::Vector3, 3>& v, 
						std::array<float, 3>& p, 
						std::array<float, 3>& d, 
...
bool quad_intervals(std::array<math::Vector3, 4>& v, std::array<float, 4>& p, std::array<float, 4>& d, float& t1, float& t2, math::Vector3& p1, math::Vector3& p2)
{
	/*
	...
	else throw std::exception("quad_intervals: Degenerate quadrilateral!");
...
bool math::TriangleQuadIntersection(Triangle& triangle, Quadrilateral& quad, Vector3& P0, Vector3& P1)
{
	...
	triangle_intervals(tv, tp, td, tt0, tt1, tp0, tp1);


	// Compute interval and intersection points for quadrilateral
	std::array<Vector3, 4> qv = { quad.v0, quad.v1, quad.v2, quad.v3 };
	quad_intervals(qv, qp, qd, qt0, qt1, qp0, qp1);


	// Sort intervals
	if (tt0 > tt1) std::swap(tt0, tt1), std::swap(tp0, tp1);
	if (qt0 > qt1) std::swap(qt0, qt1), std::swap(qp0, qp1);

	// Intervals do not overlap, so no intersection occurs
	if (tt1 < qt0 || qt1 < tt0) return false;
	...
	{
		case AXIS_X:
			if (direction.x < 0 && P0.x < P1.x) std::swap(P0, P1);
			else if (direction.x > 0 && P0.x > P1.x) std::swap(P0, P1);
		break;

		case AXIS_Y:
			if (direction.y < 0 && P0.y < P1.y) std::swap(P0, P1);
			else if (direction.y > 0 && P0.y > P1.y) std::swap(P0, P1);
		break;

		case AXIS_Z:
			if (direction.z < 0 && P0.z < P1.z) std::swap(P0, P1);
			else if (direction.z > 0 && P0.z > P1.z) std::swap(P0, P1);
		break;
	}
