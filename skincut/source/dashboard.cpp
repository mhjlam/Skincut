...
Dashboard::Dashboard(HWND hwnd, ComPtr<ID3D11Device>& device, ComPtr<ID3D11DeviceContext>& context)
{
	...
	{
		{
			...
			(size_t)(&((CUSTOMVERTEX*)0)->pos), D3D11_INPUT_PER_VERTEX_DATA, 0
		},
		{
			"COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
			(size_t)(&((CUSTOMVERTEX*)0)->col), D3D11_INPUT_PER_VERTEX_DATA, 0
		},
		{
			"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
			(size_t)(&((CUSTOMVERTEX*)0)->tex),  D3D11_INPUT_PER_VERTEX_DATA, 0
		},
	};
	...
	// Font texture
	{
		...
		mDevice->CreateTexture2D(&texDesc, &subResource, pTexture.GetAddressOf());
...
void Dashboard::Render(std::vector<Light*>& lights)
{
	...
	ImGui::Begin("Settings");
	{
		...
		{
			ImGui::Combo("", &gConfig.RenderMode, "Kelemen/Szirmay-Kalos\0Blinn-Phong\0Lambertian reflectance");
		}
...
void Dashboard::RenderDrawLists(ImDrawList** const cmdLists, int numCmdLists)
{
	...
	{
		D3D11_MAPPED_SUBRESOURCE mappedResource;
		if (FAILED(mContext->Map(mVertexConstantBuffer.Get(), 
			0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))) return;
