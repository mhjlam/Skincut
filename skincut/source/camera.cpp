...
Camera::Camera(UINT width, UINT height, float yaw, float pitch, float distance) : 
...
void Camera::Update()
{
	...
	{
		// Update position
		float deltax = io.MouseDelta.x / mWidth;
		float deltay = io.MouseDelta.y / mHeight;
		...
		Vector4 s = Vector4::Transform(Vector4(t.x - deltax*t.w, t.y + deltay*t.w, t.z, t.w), transform.Invert());
