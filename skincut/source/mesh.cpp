...
Mesh::Mesh(const std::wstring& meshname)
{
	...
	// Binary file name
	std::wstring binname = meshname + std::wstring(L".bin");
...
void Mesh::ParseMesh(const std::wstring& meshname, bool computeNormals)
{
	std::stringstream contents;

	HMODULE mod = GetModuleHandle(nullptr);
	HRSRC hrsrc = FindResource(mod, meshname.c_str(), RT_RCDATA);
	...
	else // try to load from file
	{
		...
		else // not found
		{
			...
			std::wstring meshname2 = std::wstring(L"resources\\") + meshname;
	...
	if (!contents) throw std::exception("Mesh loading error: Unable to read mesh file.");
	...
	for (uint32_t i = 0; i < indexers.size(); i+=3)
	{
		...
		float r = 1.0f / (s1 * t2 - s2 * t1); if (!_finite(r)) r = 0.0f;

		// tangent points in the u texture direction
		Vector4 tangent   = Vector4((t2 * x1 - t1 * x2) * r, 
									(t2 * y1 - t1 * y2) * r, 
									(t2 * z1 - t1 * z2) * r, 0);

		// bitangent points in the v texture direction
		Vector3 bitangent = Vector3((s1 * x2 - s2 * x1) * r, 
									(s1 * y2 - s2 * y1) * r, 
									(s1 * z2 - s2 * z1) * r);
	...
	for (auto& indexer : indexermap)
	{
		...
		float hand = math::Sign(Vector3::Dot(Vector3::Cross(vertex.normal, vertexTangent), vertex.bitangent));

		vertex.tangent = Vector4(tangent.x, tangent.y, tangent.z, hand); // store bitangent handedness in alpha channel
...
void Mesh::Subdivide(Face*& f, SPLITMODE splitmode, Vector3& p)
{
	switch (splitmode)
	{
		case SPLIT_3:
		...
		case SPLIT_4:
		{
			...
			std::array<std::pair<Face*,Edge*>,3> nbs;
			Neighbors(f, nbs);
...
bool Mesh::RayIntersection(math::Ray& ray, Intersection& intersection)
{
	...
	for (auto face : mFaceArray)
	{
		...
		// Test whether the ray passes through the triangle face.
		float t, u, v;
		if (math::RayTriangleIntersection(ray, 
			Triangle(v0.position, v1.position, v2.position), t, u, v))
		{
			hit = true;
			...
			if (t < tmin)
			{
				...
				intersection.pos_os = ray.origin + (t * ray.direction);
				intersection.pos_ts = Vector2::Barycentric(v0.texcoord, v1.texcoord, v2.texcoord, u, v);
...
void Mesh::FuseCutline(std::list<Link>& cutline, std::vector<Edge*>& cutedges)
{
	...
	for (auto l = cutline.begin(); l != cutline.end(); ++l)
	{
		...
		if (n0 = N(p0, f))
		{
			...
			if (n1 = N(p1, f))
			{
				...
				// p0->p1 is f->e[0,1,2]
				Edge* ec = nullptr;
				if      (n0 == f->n[0]) ec = (n1 == f->n[1]) ? f->e[0] : f->e[2];
				else if (n0 == f->n[1]) ec = (n1 == f->n[0]) ? f->e[0] : f->e[1];
				else if (n0 == f->n[2]) ec = (n1 == f->n[0]) ? f->e[2] : f->e[1];
				else throw std::exception("Mesh degeneracy detected!");
			...
			else
			{
				...
				Edge* ec = nullptr;
				if      (n0 == ec0->p[1].first) ec = ec0;
				else if (n0 == ec1->p[1].first) ec = ec1;
				else if (n0 == ec2->p[1].first) ec = ec2;
				else throw std::exception("Mesh degeneracy detected!");
		...
		else
		{
			...
			if (n1 = N(p1, f))
			{
				...
				// one of the splitters lies on the cutting line
				Edge* ec = nullptr;
				if      (n1 == ec0->p[1].first) ec = ec0;
				else if (n1 == ec1->p[1].first) ec = ec1;
				else if (n1 == ec2->p[1].first) ec = ec2;
				else throw std::exception("Mesh degeneracy detected!");
	...
	for (auto edge : sides) delete edge;
}
...
void Mesh::OpenCutline(std::vector<Edge*>& EC, math::Quadrilateral& cutquad, bool gutter)
{
	...
	float L = 0.0f;
	for (auto e : EC) L += Vector3::Distance(e->n[0]->p, e->n[1]->p); L *= 20; // convert to cm
	float d = std::max(0.1f, std::min(1.0f, 0.2f*L)); // min: 0.1cm (0.005 units), max: 1cm (0.05 units)

	float dsteps = (d-0.1f)/0.02f;
	float cod = (0.0111f + (0.0002f * dsteps)) * std::logf(L) + (0.0415f + (0.0015f * dsteps));
	//float cod = (0.01f*std::logf(L)+0.04f)/20.f; //float cod = 0.005f * std::sqrtf(L);

	// convert to object-space units (1cm = 0.05 units)
	d /= 20.0f;
	cod /= 20.0f;
	float halfcod = cod*0.5f;
	...
	std::function<float(float x)> codpi = [=](float x) -> float
	{
	...
	for (uint32_t i = 0; i < nEC; ++i)
	{
		...
		float cod0 = halfcod * codpi(float(i)   / (float)nEC);
		float cod1 = halfcod * codpi(float(i+1) / (float)nEC);
		...
		{
			// new positions
			Vector3 p0i = p0 + inward * d;
			Vector3 p1i = p1 + inward * d;
	...
	std::function<void(Face*& f, std::unordered_set<Face*, FaceHash, FaceHash>& table)> add_face = 
		...
		for (auto e : EC)
		{
			for (uint8_t i = 0; i < 3; ++i)
			{
				...
				if (f->v[i] == e->p[0].second || f->v[i] == e->p[1].second)
				{
					...
					if (nbs[0]) add_face(nbs[0], table);
					if (nbs[1]) add_face(nbs[1], table);
					if (nbs[2]) add_face(nbs[2], table);
...
void Mesh::ChainFaces(std::list<Link>& chain, std::map<Link, std::vector<Face*>>& cf, float r)
{
	...
	for (auto& link : chain) neighborwalk(link, link.f);
...
void Mesh::ChainFaces(std::list<Link>& chain, std::map<Link, std::vector<Face*>>& cf_outer, std::map<Link, std::vector<Face*>>& cf_inner, float r_outer, float r_inner)
{
	...
	std::function<void(Face*, LinkFaceMap&)> associate_faces = [&](Face* f, LinkFaceMap& cf)
	{
		Link l_min;
		float d_min = std::numeric_limits<float>::max();
	...
	// find all faces nearest to each link
	for (auto l : chain) neighborwalk(l, l.f);
	
	// associate each face surrounding cutline with closest link
	for (auto f : f_outer) associate_faces(f, cf_outer);
	for (auto f : f_inner) associate_faces(f, cf_inner);
}


void Mesh::Neighbors(Face*& f, std::array<Face*,3>& nbs)
{
	for (uint8_t i = 0; i < 3; ++i)
	{
		if      (f->e[i]->f[0] == f) nbs[i] = f->e[i]->f[1];
		else if (f->e[i]->f[1] == f) nbs[i] = f->e[i]->f[0];
		else throw std::exception("Degenerate mesh detected!");
	}
}


void Mesh::Neighbors(Face*& f, std::array<std::pair<Face*,Edge*>,3>& nbs)
{
	for (uint8_t i = 0; i < 3; ++i)
	{
		if      (f->e[i]->f[0] == f) nbs[i] = std::pair<Face*, Edge*>(f->e[i]->f[1], f->e[i]);
		else if (f->e[i]->f[1] == f) nbs[i] = std::pair<Face*, Edge*>(f->e[i]->f[0], f->e[i]);
		else throw std::exception("Degenerate mesh detected!");
	}
}
...
void Mesh::Split2(Face*& f, Edge*& es, math::Vector3& p, Edge** ec)
{
	//       n_1
	//       /|\ 
	//  e_1 / | \ e_2
...
void Mesh::Split3(Face*& f, math::Vector3& p, Edge** ec0, Edge** ec1, Edge** ec2)
{
	//       n_1
	//       / \ 
	//  e_0 /   \ e_1
...
uint32_t Mesh::MakeVertex(Vector3& p, Vector2& x, Vector3& n, Vector4& t, Vector3& b)
{
	...
	if (!entry.second) index = entry.first->second; // already exists
	else mVertexes.push_back(vertex);
...
Node* Mesh::MakeNode(Vector3& p)
{
	...
	else mNodeArray.push_back(node);
...
Edge* Mesh::MakeEdge(Node*& n0, Node*& n1)
{
	...
	if (edge->n[1]->p < edge->n[0]->p) std::swap(edge->n[0], edge->n[1]);
	...
	else mEdgeArray.push_back(edge);
...
Face* Mesh::MakeFace(Node*& n0, Node*& n1, Node*& n2, uint32_t i0, uint32_t i1, uint32_t i2)
{
	...
	else mFaceArray.push_back(face);
